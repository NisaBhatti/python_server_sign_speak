...
# Try the new import path (mediapipe >= 0.10.15)
try:
    ...
    try:
        from mediapipe.python.solutions import hands as mp_hands_module
        print("✅ Using fallback import")
    except ImportError:
        # Last resort - use old API if it exists
...
def detect():
    try:
        ...
        return jsonify({
            'hasHand': True, 'isAlphabet': is_alphabet, 'confidence': score,
            'landmarks': landmarks, 'alphabet': alphabet,
            'display': ALPHABET_DISPLAY.get(alphabet, alphabet),
            'message': 'Success'
        })
